...
const PowerupManager = () => {
  ...
  const toggleFavorite = (id) => {
    setPowerups(
      powerups.map((p) => (p.id === id ? { ...p, favorite: !p.favorite } : p))
    );
  };
  ...
  const saveEdit = () => {
    setPowerups(
      powerups.map((p) => (p.id === editingId ? { ...p, ...editedPowerup } : p))
    );
    setEditingId(null);
    setEditedPowerup({
      name: "",
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-900 via-green-800 to-black p-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="space-y-3">
          {filteredPowerups.length === 0 ? (
            <div className="bg-black/40 backdrop-blur-sm rounded-xl p-8 text-center border border-green-500/30">
              <p className="text-green-400 text-lg">{t("noPowerupFound")}</p>
            </div>
          ) : (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <p className="text-green-200 mb-3">{powerup.effect}</p>
                    <div className="flex flex-wrap gap-2">
        ...
        <div className="mt-6 text-center text-green-400 text-sm">
          {filteredPowerups.length} {t("displayedCount")} {powerups.length}{" "}
          {t("total")}
        </div>
      </div>
    </div>
...
ReactDOM.render(<PowerupManager />, document.getElementById("root"));
